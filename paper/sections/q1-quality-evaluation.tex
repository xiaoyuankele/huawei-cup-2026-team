%% 可直接由仓库 paper/main.tex 通过 \input{sections/q1-quality-evaluation.tex} 引入。
...
\begin{figure}[htbp]
  ...
  \caption{各领域样本质量评分分布。轮廓表示按域峰值归一化的密度，箱体表示第 25--75 百分位数，竖线为中位数，菱形为均值，空心圆为箱须界限外观测；密度高度不表示样本量。}
...
\begin{figure}[htbp]
  ...
  \caption{各领域的质量指标画像。单元格为全量去重样本在相应领域、指标上的归一化均值，全图共用 0--1 色标；三项负向指标已逆向。}
...
\begin{figure}[htbp]
  ...
  \caption{$A_1$ 与全量去重语料的领域均分对照。左图中空心圆为 $A_1$，实心菱形为全量均分，并标注样本量变化；右图为全量减 $A_1$ 的均分差值。}
...
\begin{figure}[htbp]
  ...
  \caption{领域构成变化与总体均分分解。左图为 $A_1$ 与全量语料的领域占比；右图按先替换领域均分、再替换领域占比的顺序，将总变化分为域内变化与构成变化。}
...
\begin{figure}[htbp]
  ...
  \caption{不同评价方案的排序敏感性。三个面板依次展示与主模型的 Spearman 相关系数、平均绝对百分位排名变化，以及排名变化至少 20 个百分点的样本比例。}
