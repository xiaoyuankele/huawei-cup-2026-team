...
图~\ref{fig:q3-m2p-interface}把表~\ref{tab:q3-sp3-representative}中的五种
映射按假设 $Q_B$ 的递增顺序连接在 $N^*$--$D^*$ 平面上。由 partial raw 到
interval high，点沿着“更高的 $N^*$、更低的 $D^*$”方向移动，同时质量成本
占比上升；这与表中的 $N^*$ 从 8.094 增至 8.630、$D^*$ 从 192.756 降至
171.916 相一致。连线只表示映射情景的排序，不能被读作观测样本的时间轨迹或
配比变化的因果路径。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.80\textwidth]{figures/q3-m2p-interface-cn-v1/fig_q3_m2p_interface_cn_single_v2.pdf}
  \caption{代表性预算--上下文下 M2/P 假设映射造成的条件资源配置变化。点的颜色表示假设 $Q_B$，连线表示五种映射情景的排序；图形不构成 $Q_A$--$Q_B$ 的实证关系。}
  \label{fig:q3-m2p-interface}
\end{figure}

从子问题三的接口评价看，图~\ref{fig:q3-m2p-interface}把表中的数值变化转成了可检查的
方向关系：在代表性情景下，假设 $Q_B$ 从 0.100 增至 0.943 时，$N^*$ 从 8.094 增至
8.630，约增加 6.6\%；$D^*$ 从 192.756 降至 171.916，约减少 10.8\%；质量成本
占比则由 0 增至 0.049。因而图形直接支持“接口标度假设会改变资源配置”的条件结论，
也使 $Q_A\to Q_B$ 映射对规模与数据量的方向性影响可以被复核。

这张图的评价边界同样明确：横纵坐标上的点来自同一预算--上下文切片，颜色和连线编码的
是映射假设的顺序，而非新增观测或时间演化。若改变映射函数、截断区间或质量成本，点的
位置和连线斜率都可能变化；因此该图适合用于接口压力测试和敏感性比较，不适合用于
证明 $Q_A$ 与 $Q_B$ 的实证关系、配比因果效应或唯一最优配比。

图~\ref{fig:q3-m2p-interface}的横轴是最优参数量 $N^*$，纵轴是最优训练数据量 $D^*$，
颜色条编码假设的 $Q_B$，点旁标签给出五种映射情景，箭头只表示从较低到较高假设
质量的排序。图中各点沿右下方向移动，意味着在相同代表性预算和上下文下，提高假设的
$Q_B$ 会牺牲一部分 $D^*$、增加一部分 $N^*$，同时承担更多质量成本；这与表中质量成本
占比从 0 增至 0.049 的变化一致。由于点没有围绕回归线的散布，也没有观测误差条，
图形表达的是模型接口内部的资源重新分配，而不是数据中的相关性估计。

从问题三的回答结构看，这张图把“配比如何进入 B 侧”拆成了两个可核查步骤：首先由
配比得到 A 侧标签 $Q_A(p)$，再由假设映射得到 B 侧 $Q_B$ 并调用 M1 优化器。图中的
五个点只对应第二步后的条件结果；横轴没有直接绘制 $Q_A$，因此不能把点的右移解释为
观测配比本身导致参数增加。只有在明确给定映射、成本、预算和上下文后，才能复现图中的
方向和数值；更换任一接口假设都应重新生成整张条件面板。
