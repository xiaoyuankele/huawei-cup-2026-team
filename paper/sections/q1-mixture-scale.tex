...
\begin{figure}[htbp]
  ...
  \caption{17 个训练领域的平均配比及配方分布范围。右图线段为 P5--P95 分位区间，并非均值置信区间；10B、70B 表中的 Loss 为给定估算值。}
...
\begin{figure}[htbp]
  ...
  \caption{A6/A7 上线性与二阶 ilr-Ridge 的领域间有限替代效应。行表示增加配比的领域，列表示减少配比的领域；每格为 13 个目标预测变化的等权均值。}
...
\begin{figure}[htbp]
  ...
  \caption{二阶模型相对线性模型的同尺度误差收益，以及独立历史运行中的领域替代方向一致率。区间仅反映固定模型下的配方重采样波动。}
...
\begin{equation}
\hat L_k(\boldsymbol p,S)=f_k\!\left[\operatorname{ilr}(\boldsymbol p)\right]+c_k(\boldsymbol p,S),\qquad c_k(\boldsymbol p,1\mathrm M)=0.\label{eq:q13-scale}
\end{equation}
...
\begin{figure}[htbp]
  ...
  \caption{两类配比模型在五组评价表上的 pooled RMSE 与逐目标平均 MAE。灰色区域的 10B、70B 结果依据给定估算 Loss，仅作外推情景诊断。}
  \label{fig:q13-scale}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{A10/A11 上三类修正相对于无修正的逐目标 MAE 降幅，以及 10B 到 70B 的给定与预测 Loss 降幅。高规模比较不是实测训练验证。}
...
\begin{figure}[htbp]
  ...
  \caption{加入 $Q_{\mathrm{proxy}}$ 后的 RMSE、MAE 差值及配方成对重采样的 95\% 描述性区间。负值表示误差下降；重复的 1M 点不计为独立验证。}
...
\begin{figure}[htbp]
  ...
  \caption{A8/A9 的五折校准内与折外误差对照。图中的点是全表汇总误差，不是五折均值及标准差。}
  \label{fig:q13-oof}
\end{figure}
