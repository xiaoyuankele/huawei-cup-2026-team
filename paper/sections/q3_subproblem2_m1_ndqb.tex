...
中点、高值以及不同的 $Q_B$ 初始值；保留优化器收敛、预算可行且当前情景目标值最小的
...
图~\ref{fig:q3-m1-qb}显示低预算代表性切片下三类质量成本对 $Q_B^*$ 的影响。
在相同的五档上下文中，成本曲线的差别会改变质量变量是否较早触及 $Q_B=1.0$
上界；这说明质量投入与规模、数据量共享预算时，成本形状本身就是资源配置的
一个条件因素。该图只抽取 $C=10^{19}$ FLOPs 的 15 行，不能替代前述三档预算
共 45 行的情景表，也不能把 $Q_B^*=1$ 解释为经验验证的质量饱和点。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{figures/q3-m1-qb-cn-v1/fig_q3_m1_qb_cn_single_v2.pdf}
  \caption{M1 低预算代表性情景下的最优原生质量变量 $Q_B^*$。$Q_B$ 为 B6/B7 原生变量；曲线比较三类假设质量成本和五档上下文。}
  \label{fig:q3-m1-qb}
\end{figure}

图~\ref{fig:q3-context-cost}给出上下文成本转移的结构解释。比值曲线在
$L_{\rm ctx}=30000$ 处穿过 1，正好对应式~\eqref{eq:q3-sp2-context-ratio}；
观测上下文集合中的 $8192$ 位于训练成本占优的一侧，而 $32768$ 和 $131072$
位于注意力成本占优的一侧。因此，图形支持的是成本项的代数分解和情景解释，
不支持把 $30000$ 认作模型效果已经发生转折的实证阈值。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.78\textwidth]{figures/q3-context-cost-cn-v1/fig_q3_context_cost_cn_single_v1.pdf}
  \caption{上下文注意力成本与训练成本的结构比值。虚线标出 $L_{\rm ctx}=30000$ 的成本结构临界点；该点不表示模型效果验证的临界点。}
  \label{fig:q3-context-cost}
\end{figure}

结合子问题二的目标，图~\ref{fig:q3-m1-qb}说明质量成本函数的形状会直接改变最优原生
$Q_B$，且这种影响在低预算情景中比上下文档位之间的细小差异更明显：指数型成本在该
切片中贴近 $Q_B=1$ 上界，幂函数型和对数渐进型则保留在较低的质量区间并随上下文
缓慢上升。这一现象支持“质量成本是资源分配条件”的判断，但只代表 $C=10^{19}$ FLOPs
的 15 行切片；完整结论仍须以 45 行输出及其边界状态为准。

图~\ref{fig:q3-context-cost}对上述现象给出了成本层面的解释。其纵轴是由成本定义直接
推出的结构比值，因此可复核 $L_{\rm ctx}=30000$ 两侧训练项与注意力项的主导关系；它
不能提供质量提升的统计证据，也不能替代不同质量成本函数之间的情景比较。两幅图合在
一起的评价是：M1 可以识别给定成本假设下的条件替代关系，但不能把某一成本函数下的
最优 $Q_B$ 解释为已被观测数据确认的质量上限。

图~\ref{fig:q3-m1-qb}的横轴是上下文长度，纵轴是低预算切片下的最优原生质量
$Q_B^*$，三种颜色和线型分别对应三种质量成本函数。指数型成本曲线在五个上下文
档位均贴近 $Q_B=1$，说明在该成本设定下，优化器愿意把低预算中的较大份额用于质量
变量；幂函数型和对数渐进型曲线则位于较低区间，并随上下文增加而上升。图右侧标出的
代表性末端值约为 0.692 和 0.781，说明成本函数之间的差异大于同一成本函数内由上下文
档位带来的局部变化。这里的“差异”是最优条件值的差异，不是三种质量成本在真实训练
中的效果差异。

图~\ref{fig:q3-context-cost}使用对数坐标展示 $C_{\rm attn}/C_{\rm train}$ 随
$L_{\rm ctx}$ 的变化。实线按 $L_{\rm ctx}/30000$ 增长，水平虚线是比值为 1 的位置，
竖线标出结构临界点；因此 $8192$ 位于训练成本占优区间，$32768$ 和 $131072$ 位于
注意力成本占优区间。该图与图~\ref{fig:q3-m1-qb}的关系是“成本结构解释质量配置”：
前者说明上下文如何改变预算分解，后者说明在给定质量成本函数下预算分解如何改变
$Q_B^*$。两图都是确定性情景计算，没有误差条或重复实验分布，不能据此进行统计显著性
判断。
