...
从问题三子问题一的判定角度看，图~\ref{fig:q3-m0-nd-baseline}和图~\ref{fig:q3-m0-nd-budget}
共同回答了“上下文成本改变后，预算优先落在参数规模还是数据规模上”这一问题：在同一
预算下，$L_{\rm ctx}$ 增大使可用于 $ND$ 的有效预算降低，最优点整体向较小的规模--数据
乘积移动；在预算继续增大时，支持域上界接管最优解，资源配置由内部替代关系转为边界
约束关系。因此，图形所支持的是“上下文成本与观测支持域共同决定条件分配”的结论，
而不是单独由几何趋势推出新的标度律。

图形评价还表明，M0 的主要敏感性来自约束激活而非图线的平滑外观。低预算点的解析解
与有界解接近，说明在该区域约束对结果影响较小；中、高预算点出现空心点与实心点分离，
则直接标出无界解析解已经离开 B1 支持域。因而这些图可以作为优化器和支持域检查的
可视化复核，但不能作为支持域外 Loss 或模型效果的经验验证。

具体读图时，图~\ref{fig:q3-m0-nd-baseline}的横轴为最优参数量 $N^*$、纵轴为最优
训练数据量 $D^*$；不同颜色对应五档 $L_{\rm ctx}$，空心点是未施加支持域上界的
解析配置，实心点是施加边界后的配置，方形则表示至少一个上界已经活跃。这样，空心点
与实心点的间距直接给出“解析解需要被约束修正”的程度，而不只是显示一个最优点的
位置。低预算时两类点几乎重合，说明资源尚未被 B1 上界截断；预算升高后，点逐渐贴近
$N_{\max}$ 或 $D_{\max}$，表明继续增加名义预算并不会在当前证据范围内转化为可用的
$N$ 或 $D$ 增长。

图~\ref{fig:q3-m0-nd-budget}则把同一判定展开到 15 个预算--上下文组合：上图追踪
$N^*$，下图追踪 $D^*$，横轴为三档计算预算，每条线代表一个上下文长度。在
$10^{22}$ FLOPs 档，短上下文的 $D^*$ 已接近或触及 $D_{\max}$，而较长上下文因
注意力开销更高而保持较低的有效规模；在 $10^{24}$ FLOPs 档，不同上下文的曲线都在
支持域双上界处汇合。由此，图形不仅显示“规模和数据量如何变化”，还显示了内部解、
单边界解和双边界解之间的状态转换；这种状态转换正是 M0 有界优化相对于无界解析
公式新增的信息。
